...
Welcome to the Payment Search Challenge! This is a frontend coding challenge
designed to assess your ability to implement a payment search feature using
...
> **Note**: We don't expect candidates to complete all steps. Please work your
> way through the steps incrementally, focusing on quality over quantity. Each
> step builds upon the previous one, so take your time to implement each feature
> properly.
...
> **Note**: You are free to use additional libraries, packages, or technologies
> to solve this challenge. Feel free to install and use any dependencies that
> will help you implement the features more effectively or improve the user
> experience.

> **Important**: You'll discuss your implementation in a technical interview,
> including your architectural decisions, trade-offs, and alternative approaches
> you considered. Make sure you understand every line of code you submit.

## 📡 API Documentation

The project uses MSW to mock the payment search API. The API URL is defined in
`src/constants/index.ts` as `API_URL = "/api/payments"`.

### Search Payments Endpoint

**Endpoint:** `GET /api/payments`

**Description:** Search and retrieve payments with filtering and pagination
support.
...
- `pay_134_1`, `pay_134_2`, `pay_134_3`, `pay_134_4`, `pay_134_5` - Various
  payment statuses
- `pay_205_1`, `pay_205_2`, `pay_205_3`, `pay_205_4`, `pay_205_5` - Multiple
...
**Important**: Use the i18n constants for all user-facing text. All strings can
be found in the constants files:

- **Main i18n strings**: `src/constants/i18n.ts` - Contains all user-facing text
  including labels, buttons, messages, and table headers
...
**Note**: These strings are used by our tests and scoring system, so using the
exact i18n constants is required for your solution to pass validation.
...
- Don't hesitate to install additional packages if they help you solve the
  problem more efficiently
- Consider using libraries for state management, form handling, or UI components
  if they improve your solution

## 📋 Implementation Steps

This challenge is designed to be completed incrementally. Each step builds upon
the previous one, and you can run tests to verify your progress.
...
![Payments Page](./docs/payments_page.png) _The main payments interface showing
the search form, filters, and payment table_
...
![Search by Payment ID](./docs/search_by_payment_id.png) _Search input for
payments by payment ID_
![Search by Payment ID](./docs/search_by_payment_id_with_search.png) _Searching
for payments by payment ID with results displayed in the table_
...
![Clear Filters](./docs/clear_filters.png) _Clear Filters button_
...
![Payment Not Found](./docs/payment_not_found.png) _Error message displayed when
a payment is not found (404 error)_
...
![Internal Server Error](./docs/internal_server_error.png) _Error message
displayed when a server error occurs (500 error)_
...
![Filter by Currency](./docs/filter_by_currency.png) _Filtering payments by
currency using the dropdown selector_
...
- Display pagination controls (Previous/Next buttons) (use the
  [i18n strings for the buttons](./src/constants/i18n.ts) the icons are unicode
  characters in the i18n strings)
...
![Paginate Payments](./docs/paginate_payments_page_1.png) _Pagination controls
allowing navigation between pages of results_

![Paginate Payments](./docs/paginate_payments_page_2.png) _Pagination controls
allowing navigation between pages of results_
...
This section documents my implementation of the Payment Search Challenge,
focusing on the features delivered, architectural decisions, and the tools added
...
- Renders payment data in a structured table.

- Uses i18n constants for all headers and labels.

- Formats amounts and dates consistently.

#### Search by Payment ID

- Search input with correct ARIA label.

- Search button triggers committed filter updates.

- Table refreshes based on the search term.

#### Clear Filters

- Clear button appears only when filters are active.

- Resets search, currency, and pagination.

- Ensures UI state and query state remain in sync.

#### Error Handling

- 404 “payment not found” state.

- 500 “server error” state.

- Error messages displayed using i18n constants.
...
- Search + currency filters applied together.

- API receives both parameters.

- Table displays only matching results.

#### Pagination

- Next/Previous buttons with correct ARIA labels.

- Page label updates (“Page 1”, “Page 2”, etc.).

- Buttons enable/disable based on page.

- Table updates with correct page data.

### Architectural Decisions

#### Committed Filter State

I used committed filter state (search, currency, page) instead of uncontrolled
inputs to ensure:
...
- clear separation between UI state and query state
...
All interactive elements use ARIA labels from i18n constants. This ensures:

- screen reader compatibility

- predictable selectors for testing

- alignment with challenge requirements

### End-to-End Testing (Playwright)

I added a Playwright E2E test suite to validate the entire user flow in a real
browser environment.
...
- stable selectors (getByTestId, getByRole)

- realistic browser execution
...
Vitest is used for unit testing React Query hooks and utility logic.

#### Mocking Strategy

- API calls are mocked using vi.mock()

- Named exports (e.g., fetchPayments) are replaced with vi.fn()
...
This implementation delivers a complete, accessible, and fully tested payment
...
The documentation above focuses exclusively on the work I implemented, without
repeating any of the challenge’s original instructions or pre-installed tooling.
